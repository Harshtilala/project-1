    ...
    <main class="main-content" style="flex: 1; padding: 20px; overflow-y: auto; background-color: #f8f9fa;">
        <div class="container-fluid">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="title" style="color: black; font-weight: bold; font-size: 28px;">Edit Stock</h2>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-body">
                    <form action="{{ route('stockstatus.update', $stockStatus->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="item_name" id="item_name" class="form-control"
                                    value="{{ old('item_name', $stockStatus->item_name) }}" required>
                                @error('item_name')
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="category" id="category" class="form-select" required>
                                    ...
                                    <option value="gold" {{ old('category', $stockStatus->category) == 'gold' ? 'selected' : '' }}>Gold</option>
                                    <option value="silver" {{ old('category', $stockStatus->category) == 'silver' ? 'selected' : '' }}>Silver</option>
                                    <option value="diamonds" {{ old('category', $stockStatus->category) == 'diamonds' ? 'selected' : '' }}>Diamonds</option>
                                    <option value="jewelry" {{ old('category', $stockStatus->category) == 'jewelry' ? 'selected' : '' }}>Jewelry</option>
                                </select>
                                @error('category')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.01" name="weight" id="weight" class="form-control"
                                    value="{{ old('weight', $stockStatus->weight) }}" required>
                                @error('weight')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="purity" id="purity" class="form-control"
                                    value="{{ old('purity', $stockStatus->purity) }}" required>
                                @error('purity')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="quantity" id="quantity" class="form-control"
                                    value="{{ old('quantity', $stockStatus->quantity) }}" required>
                                @error('quantity')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
